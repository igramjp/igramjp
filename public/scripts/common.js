...
function log(num) {
  ...
  if (num == 1) {
    ...
    message += "音楽に特化した高音質ビデオチャット。ノイキャン・エコーキャンセル・ノイズ抑制をすべてオフにし、320kbpsステレオで原音をそのまま伝送。ブラウザのみで即利用可能。Web会議ツールでは失われる音楽の繊細さを届ける。<br>";
    message += "High-quality video chat built for music. Delivers raw audio in 320kbps stereo with noise cancellation, echo cancellation, and noise suppression all disabled. Works instantly in browser — no app required. Preserves the subtle nuances of music that standard video conferencing tools destroy.<br>";
    message +=
